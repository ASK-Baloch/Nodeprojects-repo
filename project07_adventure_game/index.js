...
async function welcome() {
    const gameTitle = chalkAnimation.rainbow(" welcome to GTA fanmade");
    await wait();
    gameTitle.stop();
...
// Game Variables
const enemies = ["Tenpenny", "Ryder", "Big Smoke", "Andreas Sanchez"];
const maxEnemyHealth = 85;
...
const healthPotionDropChances = 50;
let running = true;
...
Game: while (running) {
    ...
    `);
    let enemyHealth = getRandomNum(0, maxEnemyHealth);
    // console.log('enemyHealth', enemyHealth);
    let enemy = enemies[getRandomNum(0, enemies.length)];
